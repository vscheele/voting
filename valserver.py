...
class webserverHandler(BaseHTTPRequestHandler):
        ...
        try:
            if self.path.endswith("/v123"):
                ...
                output+="<html>" \
                        ...
                        "<body onload=\"javascript:initform()\">Moin! FG-434 Vote."
                output += "<div id=\"formdiv\"><form method='POST' id='idform' enctype='multipart/form-data' action='/voted'><h2>" \
                          "</h2>" \
                          "Antwort 1 <input type=\"radio\" name=\"radiovote\" value=\"0\" checked=\"checked\"/><BR>"\
                          "Antwort 2 <input type=\"radio\" name=\"radiovote\" value=\"1\" /><BR>"\
                          "Antwort 3 <input type=\"radio\" name=\"radiovote\" value=\"2\" /><BR>"\
                          "<BR><BR><input type='submit' value='OK!'>"\
                          "</form></div>" \
                          "<div id='resultcontent'>start</div>"
                output+="</body></html>"
    ...
    def do_POST(self):
        if self.path.endswith("/votedajax"):
                ...
                self.wfile.write(readandformatresults())
                return
...
def readandformatresults():
    ...
    for i in range(0, len(entries)):
        ...
        result+="        <td class=\"value first\"><img src=\"bar.png\" alt=\"\" width=\""+str(10*int(entries[i]))+"\" height=\"16\" />"+entries[i]+"</td>\r\n"
        result+="</tr>\r\n"
    result+="</table>"
    f.close()
    return result
